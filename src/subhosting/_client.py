...
from ._exceptions import APIStatusError, SubhostingError
...
class Subhosting(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous subhosting client instance.

        This automatically infers the `bearer_token` argument from the `DEPLOY_ACCESS_TOKEN` environment variable if it is not provided.
        """
        if bearer_token is None:
            bearer_token = os.environ.get("DEPLOY_ACCESS_TOKEN")
        if bearer_token is None:
            raise SubhostingError(
                "The bearer_token client option must be set either by passing bearer_token to the client or by setting the DEPLOY_ACCESS_TOKEN environment variable"
            )
        self.bearer_token = bearer_token

        if base_url is None:
            base_url = os.environ.get("SUBHOSTING_BASE_URL")
        if base_url is None:
            base_url = f"https://api.deno.com/v1"
...
class AsyncSubhosting(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async subhosting client instance.
